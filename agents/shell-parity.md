...
| Workspaces | Yes | Partial — no left rail | Both shells create a workspace by name into a sibling directory, show workspace context, and forget workspaces. SwiftUI has a persistent left workspace rail (`Opt+Cmd+W`): click rebinds this window, Show Changes inspects `@` vs parent without `jj edit`, and Open in New Window is secondary. GPUI still switches via the status-bar picker / new window only — an intentional SwiftUI-only gap for this change. |
