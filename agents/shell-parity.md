...
| --- | --- | --- | --- |
| Open a Repository | Yes | Yes | Pins share `repositories.json`; Recent is shell-local. Title menu activates an open window or opens a closed pin without duplicating. GPUI falls back to a typed-path window when no desktop file picker is available. Both persist window frames and pane widths; SwiftUI's separate onboarding/list/repository scenes and Dock menu are macOS integrations. |
...
| Review Diffs | Yes | Yes | Same text/image diffs, layout modes, find, file review, and tree/flat lists. SwiftUI also persists per-hunk review state; GPUI keeps identity-based file marks. Expansion: GPUI tab-focuses per-region controls; SwiftUI has Expand All Unmodified Lines. SwiftUI tints small reveals when Reduce Motion is off; GPUI swaps atomically. Both fall back to unified for purely added or deleted files. |
...
| Tools & Settings | Yes | Partial — font-size setting | Shared config ids. Both copy the Rust-owned jj tool definition; `jayjay config` prints it. Both edit custom editor/terminal commands with arguments, and Linux defaults to the system editor and terminal (`$VISUAL`/`$EDITOR`, `$TERMINAL`, `xdg-terminal-exec`). Gap: font size is zoom-only. GPUI Settings → Tools has codex/claude; Settings → CLI has jj/gh/glab/origin plus Linux `jayjay` install (AppImage-aware symlink in `~/.local/bin`). SwiftUI-only macOS integrations are outside the GPUI Linux parity target. |
