...
| Tools & Settings | Yes | Partial | Shared config ids and option vocabularies must match. GPUI settings are cross-platform and include a Linux-only `jayjay` CLI installer (symlink in `~/.local/bin`, AppImage-aware) mirroring SwiftUI's CLIInstaller UX; macOS Help Book, Sparkle, and app-bundle CLI-installer plumbing stay SwiftUI-only. |
