...
| Navigate History | Yes | Yes | Selection, multi-change combined diffs and batch operations, revset filter, load-more, context actions, insert-before/after, drag/drop, and divergent changes stay aligned. |
...
| Workspaces | Yes | Partial — picker refresh | Both create, open, show context, search and switch through a filterable title picker, copy workspace names and paths, group sibling checkouts beneath their primary repository, forget, and Forget & Delete after confirmation. Gap: GPUI's picker has no explicit workspace refresh. |
