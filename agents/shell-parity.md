...
| Main Window | Yes | Yes | Same jj data in DAG, detail, file column, diff, status bar, markers, and workspace chips. Both cap the description preview at 80 points, expand to 320 with the file list and diff still visible, and keep the summary and Edit fixed while the body scrolls. |
...
| Review Diffs | Yes | Yes | Same text/image diffs, layout modes, find, file review, and tree/flat lists. Both resize image comparisons with a draggable divider that double-click resets. Both persist per-hunk review state from the gutter stripe with file rollups. Expansion: GPUI tab-focuses per-region controls; SwiftUI has Expand All Unmodified Lines. SwiftUI tints small reveals when Reduce Motion is off; GPUI swaps atomically. Both fall back to unified for purely added or deleted files. |
...
| Workspaces | Yes | Partial — picker refresh | Both create, open, show context, search and switch through a filterable title picker, copy workspace names and paths from the picker or a DAG `name@` chip menu, group sibling checkouts beneath their primary repository, forget, and Forget & Delete after confirmation. Gap: GPUI's picker has no explicit workspace refresh. |
...
| Tools & Settings | Yes | Yes | Shared config ids. Both copy the Rust-owned jj tool definition; `jayjay config` prints it. Both edit custom editor/terminal commands with arguments, and Linux defaults to the system editor and terminal (`$VISUAL`/`$EDITOR`, `$TERMINAL`, `xdg-terminal-exec`). Both apply the interface font-size setting. GPUI Settings → Tools has codex/claude; Settings → CLI has jj/gh/glab/origin plus Linux `jayjay` install (AppImage-aware symlink in `~/.local/bin`). SwiftUI-only macOS integrations are outside the GPUI Linux parity target. |
