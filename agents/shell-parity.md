...
| Review Diffs | Yes | Yes | Same text/image diffs, layout modes, find, file review, and tree/flat lists. Both persist per-hunk review state from the gutter stripe with file rollups. Expansion: GPUI tab-focuses per-region controls; SwiftUI has Expand All Unmodified Lines. SwiftUI tints small reveals when Reduce Motion is off; GPUI swaps atomically. Both fall back to unified for purely added or deleted files. |
