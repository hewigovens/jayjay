...
- **Typography**: `.jayjayFont(_:weight:design:)` (or `MonoFont.scaledFont` where a `Font` value is needed) in `App/Config/FontEnvironment.swift` applies the configured family and scales from the 12 pt baseline. New text styles build on it; a helper that forces the system font or adds a second scaling baseline ignores the user's settings. Some fixed-size chrome (palette, pickers, Settings) still sets system fonts directly.
