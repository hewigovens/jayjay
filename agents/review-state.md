...
- SwiftUI consumes `ReviewGroupState` / `ReviewFileRollup` through UniFFI and must not hash groups itself. File-list rollups use a current snapshot when the shell already has one so an identity-only line shift does not disagree with the gutter. GPUI currently uses file-level `is_reviewed` (identity-only marks) and does not render per-group dispositions.
