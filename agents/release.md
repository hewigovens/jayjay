...
5. Commit the version bumps, shipped user docs, SwiftUI and optional GPUI Alpha release notes, and `docs/appcast.xml` as `release: <version> (build N)`.
6. Create and push the `v<version>` tag from the release commit. Tag pushes do not build AppImages; the release-published event does, and `workflow_dispatch` on the tag builds retained CI artifacts when you want to validate the GPUI alpha before publishing. The GPUI alpha artifacts are not a release gate.
7. Run `just shell::publish` to create the public GitHub release, combining the SwiftUI notes with optional GPUI Alpha notes while keeping Sparkle's description SwiftUI-only, upload the zip, verify the Sparkle asset URL is public, and rewrite `../tap/Casks/jayjay.rb`. The AppImage workflow runs when that release is published and attaches GPUI Linux alpha AppImages plus SHA-256 files asynchronously; do not wait for it during the macOS release unless you are specifically validating GPUI alpha artifacts.
...
- The GitHub release body uses `releases/<version>.html` plus `releases/<version>-gpui.md` when that optional GPUI Alpha notes file exists. Sparkle continues to use only the SwiftUI HTML file. GPUI builds live on the same tag and GitHub release as asynchronously attached artifacts.
...
`releases/<version>.html` is mandatory, covers only the SwiftUI macOS app, and remains the sole source for the Sparkle update prompt. When the release range has user-visible GPUI changes, write `releases/<version>-gpui.md`; publication adds a Linux GPUI Alpha section to the GitHub release body without exposing those notes to Sparkle. Missing or empty SwiftUI notes abort publication. Missing GPUI notes do not block publication while AppImage artifacts remain asynchronous.
