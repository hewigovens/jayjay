...
2. Diff the complete range from the previous release tag with `jj log -r 'v<previous>..@'` and `jj diff --from v<previous> --to @ --summary`, then write the SwiftUI macOS release notes as an HTML body without wrapper tags in `releases/<version>.html`. Cover user-visible SwiftUI and shared-core changes from the whole range, not only the current local stack; omit GPUI-only changes.
...
5. Commit the version bumps, release-note file, and `docs/appcast.xml` as `release: <version> (build N)`.
...
- The GitHub release body must use the SwiftUI notes from `releases/<version>.html`.
- `../tap/Casks/jayjay.rb` must match the uploaded release asset and SHA-256.

## Release Notes

`releases/<version>.html` is mandatory and feeds both the GitHub release body and the Sparkle update prompt. It covers only the SwiftUI macOS app. The GPUI alpha package version is synchronized by `just set-version`, but do not add GPUI notes to the SwiftUI release until GPUI has its own coordinated release-note flow. Missing or empty notes abort the relevant release step.
