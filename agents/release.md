...
1. Start a release change directly on published `main`, then run `just set-version <version> <build>` to bump every source at once (`shell/justfile` version + build_number, the root Cargo workspace package version inherited by the CLI and GPUI, GPUI AppStream metadata, and `shell/mac/project.yml`). `project.pbxproj` and `Cargo.lock` regenerate on build. Never hand-edit one source — version drift ships binaries and update metadata that disagree. Both release recipes refresh `origin` and abort unless the release change's parent, local `main`, and `main@origin` are the same commit; publication repeats the check against the release tag.
2. Diff the complete range from the previous release tag with `jj log -r 'v<previous>..@'` and `jj diff --from v<previous> --to @ --summary`. From that range, refresh **shipped user docs** (below), write SwiftUI macOS notes as an HTML body without wrapper tags in `releases/<version>.html`, and write Linux GPUI Beta notes in `releases/<version>-gpui.md` when the range has user-visible GPUI changes. Cover user-visible changes from the whole range, not only the current local stack.
3. Run `just build` to verify the release version still builds.
4. Run `just release` to verify immutable worker migration checksums, build, sign, notarize, zip, verify the extracted archive with `codesign`, `stapler validate`, and `spctl -av`, produce the SHA-256, and prepend the entry to `docs/appcast.xml`. It also runs `just check-version`, aborting if any source disagrees. The base and archive checks are `scripts/verify-release.sh`; `just verify-release-base` runs the base check alone. Keep the Mac unlocked: a locked screen locks the keychain, so notarization fails with `No Keychain password item found for profile: notarytool` even when the profile exists.
5. Commit the version bumps, shipped user docs, SwiftUI and optional GPUI Beta release notes, and `docs/appcast.xml` as `release: <version> (build N)`.
6. Create and push the `v<version>` tag from the release commit. Tag pushes do not build AppImages; the release-published event does, and `workflow_dispatch` on the tag builds retained CI artifacts when you want to validate the GPUI beta before publishing. The GPUI beta artifacts are not a release gate.
7. Run `just shell::publish` to create the public GitHub release, combining the SwiftUI notes with optional GPUI Beta notes while keeping Sparkle's description SwiftUI-only, upload the zip, verify the Sparkle asset URL is public, and rewrite `../tap/Casks/jayjay.rb`. The AppImage workflow runs when that release is published and attaches GPUI Linux beta AppImages plus SHA-256 files asynchronously; do not wait for it during the macOS release unless you are specifically validating GPUI beta artifacts. The same run builds the Arch binary package (`jayjay-appimage`) for x86_64 and aarch64 from those AppImages, attaches each `.pkg.tar.zst` and its `.sha256` to the release, and for stable releases adds it to the pacman repo on Cloudflare R2 (`https://pkgs.hewig.dev/arch/$arch`); packages and the index are signed with the key in `dist/arch/jayjay-packages.asc`, which the publisher also uploads to `https://pkgs.hewig.dev/arch/jayjay-packages.asc` for `pacman-key --add` and `--lsign-key`. The repo is append-only: packages are never overwritten and the index is rebuilt from every package in the pool, so reruns and overlapping releases converge; betas stay `pacman -U` installs from the pre-release page. Pre-release tags such as `v0.3.17-beta.2` become `pkgver=0.3.17beta.2`. Secrets `R2_ACCOUNT_ID`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, and `ARCH_SIGNING_KEY` (the armored private key, no passphrase) must exist on the repository. The publisher runs the script from `main` and requires the release commit to be on `main` (step 8) before using those secrets; if it ran too early, push `main` and rerun the job.
...
10. After the AppImage workflow has attached both architectures, update the checked-in `dist/arch/PKGBUILD` version and source checksums, regenerate `dist/arch/.SRCINFO` with `makepkg --printsrcinfo`, run the workflow's Arch package lifecycle check, and submit both files to AUR. `PKGBUILD` is the maintained package source; `.SRCINFO` is generated from it, not edited independently.
...
- For GPUI Linux beta releases, the AppImage workflow attaches `jayjay-gpui-x86_64-linux.AppImage`, `jayjay-gpui-aarch64-linux.AppImage`, and matching `.sha256` files after the release is published. This is asynchronous and non-blocking for the macOS release.
...
- The GitHub release body uses `releases/<version>.html` plus `releases/<version>-gpui.md` when that optional GPUI Beta notes file exists. Sparkle continues to use only the SwiftUI HTML file. GPUI builds live on the same tag and GitHub release as asynchronously attached artifacts.
- `../tap/Casks/jayjay.rb` must match the uploaded release asset and SHA-256.
- `dist/arch/PKGBUILD` and its generated `dist/arch/.SRCINFO` must match the published AppImage release before they are submitted to AUR.
...
`releases/<version>.html` is mandatory, covers only the SwiftUI macOS app, and remains the sole source for the Sparkle update prompt. When the release range has user-visible GPUI changes, write `releases/<version>-gpui.md`; publication adds a Linux GPUI Beta section to the GitHub release body without exposing those notes to Sparkle. Missing or empty SwiftUI notes abort publication. Missing GPUI notes do not block publication while AppImage artifacts remain asynchronous.
