...
7. Run `just shell::publish` to create the public GitHub release, combining the SwiftUI notes with optional GPUI Beta notes while keeping Sparkle's description SwiftUI-only, upload the zip, verify the Sparkle asset URL is public, and rewrite `../tap/Casks/jayjay.rb`. The AppImage workflow runs when that release is published and attaches GPUI Linux beta AppImages plus SHA-256 files asynchronously; do not wait for it during the macOS release unless you are specifically validating GPUI beta artifacts. The same run builds the Arch binary package (`jayjay-appimage`) for x86_64 and aarch64 from those AppImages and attaches each `.pkg.tar.zst` to the release; pre-release tags such as `v0.3.17-beta.2` become `pkgver=0.3.17beta.2`, which pacman orders before the final version. Once that workflow has finished, run `just shell::publish-arch` to refresh the pacman repos hosted on the rolling `arch-repo-x86_64` / `arch-repo-aarch64` releases (`Server = https://github.com/hewigovens/jayjay/releases/download/arch-repo-$arch`, unsigned, so users set `SigLevel = Optional TrustAll`). It runs `dist/arch/publish-repo.sh` inside the OrbStack Arch machine `arch-pkg` (`orb create arch arch-pkg`, then `sudo pacman -Sy github-cli` in it); the script uploads the package first, swaps each index into place, and refuses to downgrade the index.
