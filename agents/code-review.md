...
1. Read `AGENTS.md`.
2. Load the focused guide for the changed area from the `AGENTS.md` Start Here list (`architecture`, `storage`, `swiftui`, `gpui`, `shell-parity`, `review-state`, `format-projections`, `testing`, `design`, `help-book`, `pull-requests`, `version-control`, or `release`).
