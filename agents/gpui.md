...
├── startup/       Clap launcher arguments, typed GUI launch requests, shared CLI dispatch,
│                  and Linux process detachment; help/version/config/review exit before GPUI init
│                  (app/runtime.rs initializes GPUI; app/startup_window.rs opens the initial window)
...
- **Theme** (`app/theme/`): every color is derived in `derive.rs` from a `jayjay_core::ThemeSeed` (background, foreground, accent, and a few status hues); the built-in light and dark themes are seeds too, so a new color gets a derivation rule, never a per-palette literal.
- **Typography**: size text with `ui_font_size(base)` (`app/theme.rs`), a rem value; `theme_for_window` sets the window rem size from the configured font size, so it scales. Use `Theme::scaled_font_size` for pixel math (line heights, control heights) and `app::fonts::mono()` for the configured code family. A literal `px` text size ignores the user's setting.
- **Globals** (the only ones): `Theme`, `AppConfigStore` (TOML config), `repositories::StoreHandle` (the shared Rust-backed project pins), `ReviewStoreHandle` (process-wide review store — mutate only through `window/review.rs::mutate`), `links::UrlOpener` (the injectable URL boundary every link and the feedback prompt go through, defaulting to `platform::open_url`, which runs the system handler with stdio detached), and the test-only `WatcherSuppressed`. Component tests install ephemeral config, light theme, in-memory pin/review stores, and a suppressed watcher through `tests/support::install_test_globals`; the GPUI harness installs a recording opener through `links::install_url_opener` and tests read it back with `harness::opened_url`.
...
- **Dependencies**: `gpui` and `gpui_platform` are pinned to one `zed-industries/zed` stable release tag (`tag = "vX.Y.Z"`, never a bare commit); bump all three entries in `shell/gpui/Cargo.toml` together and run `just test-gpui`. Crates built against a different gpui git source have incompatible types, so third-party gpui component libraries cannot be added as dependencies; vendor the specific behavior into `ui/` instead.
