...
- **Globals** (the only ones): `Theme`, `AppConfigStore` (TOML config), `repositories::StoreHandle` (the shared Rust-backed project pins), `ReviewStoreHandle` (process-wide review store — mutate only through `window/review.rs::mutate`), and the test-only `WatcherSuppressed`. Tests replace all of them via `tests/support::install_test_globals` (ephemeral config, light theme, in-memory pin/review stores, suppressed watcher).
