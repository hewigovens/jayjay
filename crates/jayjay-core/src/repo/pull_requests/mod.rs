...
use super::Repo;
use super::hosted_repo::HostedRepo;
...
impl Repo {
    ...
    pub fn pull_request_open_url(&self, bookmark: &str) -> Option<String> {
        ...
        let base = if remote.needs_pull_request_base() {
    ...
    pub fn pull_request_host_name(&self) -> Option<String> {
        let remote = self.git_remote_url().ok()?;
        let remote = HostedRepo::parse(&remote)?;
        Some(remote.display_name().to_owned())
    }
    ...
    fn pull_request_info_for_remote(&self, remote: &HostedRepo, bookmark: &str) -> Option<PrInfo> {
        if remote.is_github() {
            self.gh_pr_info(bookmark)
        } else if remote.is_codeberg() {
            codeberg::pr_info(self, remote, bookmark)
        } else {
            None
        }
    }
