use jayjay_primitives::{
    ReviewDiffProvider, ReviewFileDiff, ReviewHunk, ReviewNoteStatus, ReviewResult,
};
...
impl Repo {
    ...
    /// Reconcile against an explicit store — the seam for the CLI (which also needs the resolved change id) and for tests with non-global stores.
