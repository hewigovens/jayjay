...
) -> (Entity<ExternalToolWindow>, &mut VisualTestContext) {
    install_test_globals(cx);
    let (view, cx) = cx.add_window_view(|_, cx| ExternalToolWindow::new(invocation, cx));
...
fn read_only_diff_has_no_edit_or_save_actions(cx: &mut TestAppContext) {
    ...
    let (_view, cx) = open_tool(invocation, cx);
...
fn failed_read_only_diff_closes_with_an_error(cx: &mut TestAppContext) {
    ...
    let (view, cx) = open_tool(invocation, cx);
...
#[gpui::test]
fn git_merge_tool_requires_all_conflicts_to_be_resolved_before_save(cx: &mut TestAppContext) {
    let fixture = tempfile::tempdir().expect("fixture");
    let left = fixture.path().join("left.rs");
    let base = fixture.path().join("base.rs");
    let right = fixture.path().join("right.rs");
    ...
    fs::write(&output, "").expect("output");
    let generated =
        jayjay_core::external_tools::load_external_merge(&left, &base, &right, &output, false, 7)
            .expect("generate conflict markers");
    fs::write(&output, generated.result).expect("initialized merge result");
    let invocation = ExternalToolInvocation::Merge {
        left: left.to_string_lossy().into_owned(),
        base: base.to_string_lossy().into_owned(),
        right: right.to_string_lossy().into_owned(),
    ...
    assert!(
        cx.debug_bounds("external-save").is_none(),
        "Git must not stage a merge result that still contains conflict markers"
    );
    let accept = cx
        .debug_bounds("external-hunk-0-Accept Right")
        .expect("Accept Right hunk action");
    cx.simulate_click(accept.center(), Modifiers::default());
    settle_visual(cx);

    assert!(cx.debug_bounds("external-save").is_some());
}
