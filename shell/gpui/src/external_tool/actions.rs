...
use super::diff::ExternalDiffSave;
use super::merge::ExternalMergeSave;
use super::view::{ExternalToolState, ExternalToolWindow};

enum ExternalSaveRequest {
    Diff(ExternalDiffSave),
    Merge(ExternalMergeSave),
}

impl ExternalSaveRequest {
    fn run(self) -> jayjay_core::CoreResult<()> {
        match self {
            Self::Diff(request) => request.run(),
            Self::Merge(request) => request.run(),
        }
    }
}
...
impl ExternalToolWindow {
    pub(super) fn save(&mut self, cx: &mut Context<Self>) {
        ...
        let request = match &self.state {
            ExternalToolState::Diff(session) if session.editable => {
                ExternalSaveRequest::Diff(session.save_request())
            }
            ExternalToolState::Merge {
                session, result, ..
            } if session.can_save(&result.read(cx).text()) => {
                ExternalSaveRequest::Merge(session.save_request(result.read(cx).text()))
            }
            _ => return,
        };
        self.is_saving = true;
        ...
        cx.spawn(async move |this, cx| {
            let result = cx.background_spawn(async move { request.run() }).await;
            let _ = this.update(cx, move |view, cx| {
                if !view.is_saving {
                    return;
                }
                view.is_saving = false;
                match result {
                    Ok(()) => cx.quit(),
                    Err(error) => view.error_message = Some(error.to_string()),
                }
                cx.notify();
            });
        })
