...
use crate::ui::merge_editor::{
    ...
    merge_source_panel, merge_source_row,
};
...
impl ExternalToolWindow {
                ...
                div()
                    ...
                    .children(has_hunks.then(|| {
                        merge_result_mode_button("external-result-hunks", "Hunks", !raw, t)
                            .on_click(cx.listener(|view, _, window, cx| {
                                view.set_merge_result_raw(false, window, cx);
                            }))
                    }))
                    .children(has_hunks.then(|| {
                        merge_result_mode_button("external-result-raw", "Raw", raw, t).on_click(
                            cx.listener(|view, _, window, cx| {
                                view.set_merge_result_raw(true, window, cx);
                            }),
                        )
                    }))
                    .child(
    ...
    fn hunk_list(&mut self, t: &Theme, cx: &mut Context<Self>) -> AnyElement {
        let ExternalToolState::Merge {
            session, result, ..
        } = &self.state
        else {
        ...
        self.merge_hunk_list("external-hunks-scroll", cards, cx)
            ...
            .on_action(cx.listener(|view, _: &MergeUseRightHunk, window, cx| {
                view.use_selected_merge_hunk(MergeHunkSource::Right, window, cx);
            }))
            .on_action(cx.listener(|view, _: &MergePreviousHunk, window, cx| {
                view.move_merge_hunk(-1, window, cx);
            }))
