...
impl ExternalToolWindow {
    pub fn new(invocation: ExternalToolInvocation, cx: &mut Context<Self>) -> Self {
        Self::new_with_exit_state(invocation, ExternalToolExitState::default(), cx)
    }

    pub(super) fn new_with_exit_state(
        invocation: ExternalToolInvocation,
        exit_state: ExternalToolExitState,
        cx: &mut Context<Self>,
    ) -> Self {
        let mut window = Self {
            invocation,
    ...
    fn header(&self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        } else if !self.is_saving && matches!(&self.state, ExternalToolState::Merge { .. }) {
