            ...
            move |vm, result, cx| {
                ...
                match result {
                    Ok(data) => {
                        ...
                        vm.pull_request_host_name =
                            data.pull_request_host_name.map(SharedString::from);
...
struct RefreshData {
    ...
    pull_request_host_name: Option<String>,
}
...
fn refresh_graph_blocking(repo: &Repo, depth: u32) -> CoreResult<RefreshData> {
    ...
    let pull_request_host_name = repo.pull_request_host_name();
    Ok(RefreshData {
        entries,
        bookmarks,
        workspaces,
