...
impl RepoWindow {
    ...
    pub(super) fn ensure_diff_edit_files(&mut self, cx: &mut Context<Self>) {
        ...
        for hunk in hunks.iter() {
            ...
            let Some(cached) = vm
                .diff_cache
                .values()
                .find(|loaded| loaded.diff.path == path && loaded.projection.is_none())
            else {
                continue;
            };
            ...
            let ignore_whitespace = vm.ignore_whitespace;
    ...
    pub fn diff_edit_file_supported(&self, hunk: &DiffHunk, _cx: &Context<Self>) -> bool {
