...
fn header(view: &RepoWindow, t: &Theme, cx: &mut Context<RepoWindow>) -> AnyElement {
    ...
    let selecting = view.diff_edit.selecting_all;
...
fn action_bar(view: &RepoWindow, t: &Theme, cx: &mut Context<RepoWindow>) -> AnyElement {
    let working_copy = view.diff_edit_is_working_copy(cx);
    let summary = view.diff_edit_selection_text();
    let mut row = div()
        .flex()
        .items_center()
    ...
    if !working_copy {
        let input = line_input_content(
            &view.diff_edit.message,
            "New change description",
            t,
            view.diff_edit
                .message_active
                .then_some("diff-edit-message-caret"),
        );
        row = row
            .child(
                div()
                    .id("diff-edit-message")
                    .w(px(260.))
                    .px(px(7.))
                    .py(px(5.))
                    .rounded_sm()
                    ...
                    .text_size(px(11.))
                    .cursor_text()
                    .on_click(cx.listener(|view, _, _, cx| view.activate_diff_edit_message(cx)))
                    .child(input),
            )
