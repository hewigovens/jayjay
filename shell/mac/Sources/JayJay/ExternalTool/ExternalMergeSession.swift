...
final class ExternalMergeSession: MergeEditingSession {
    ...
    init(
        ...
        outputPath = output
        self.path = path
        self.outputIsInitialized = outputIsInitialized
    ...
    var canSave: Bool {
        selectedSource != nil
            || isTextMerge && (!outputIsInitialized || !hasExternalConflictMarkerRemnants(
                content: result,
    ...
    func load() async {
        ...
        let path = path
        let outputIsInitialized = outputIsInitialized
