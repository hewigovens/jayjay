...
extension RepoViewModel {
    ...
    func fetchPrInfo(bookmarks: [String]) {
        ...
        prInfo = nil
        prFetchTask = startLifecycleRepoTask { [self, repo] in
    ...
    func refresh(
        ...
        refreshTask = startLifecycleRepoTask { [self, repo] in
            do {
    ...
    func loadMore() {
        ...
        refreshTask = startLifecycleRepoTask { [self, repo, includeSubmoduleStatuses] in
            do {
                let content = try Self.loadRefreshContent(
                    repo: repo,
                    revset: nextRevset,
